...
The linux-mpu9250 code uses the InvenSense Embedded Motion Driver v6.1.2 SDK
to obtain fused 6-axis quaternion data from the MPU-9250 DMP. The quaternion
data is then further corrected in the linux-mpu9250 code with magnetometer 
data collected from the IMU.

Because it is primarily intended for use on small-board ARM computers, the 
linux-mpu9250 code uses 32-bit floats rather then doubles for all floating 
point operations.
...
We have our own boards incorporating the MPU9250s, but we also tested with the
[Sparkfun MPU9250 Breakout Boards][5]
...
Use git to fetch the linux-mpu9250 project. You may have to install <code>git</code> on your
system first.
...
Then to clone the repository assuming you have an Internet connection
        ...
        git clone https://github.com/nesiax/linux-mpu9250.git


# Build

The linux-mpu9250 code is written in C. There is a make file called <code>Makefile-native</code>
...
tools and cross-building linux-mpu9250 on a workstation. This is more for Gumstix
and Beagle users.

A recommendation is to create a soft-link to the make file you want to use.

        root@duovero:~$ cd linux-mpu9250
        root@duovero:~/linux-mpu9250$ ln -s Makefile-native Makefile
...
After that you can just type <code>make</code> to build the code.
        ...
        gcc -Wall -fsingle-precision-constant inv_mpu.o inv_mpu_dmp_motion_driver.o linux_glue.o mpu9250.o quaternion.o vector3d.o imucal.o -lm -o imucal
...
For example:
        ...
        scott@hex:~/linux-mpu9250$ export OETMP=/oe1
        scott@hex:~/linux-mpu9250$ ln -s Makefile-cross Makefile
        scott@hex:~/linux-mpu9250$ make
...
You can check with this command:
        ...
        pi@raspberrypi:~/linux-mpu9250$ dmesg | grep i2c
...
started.
        ...
        pi@raspberrypi:~/linux-mpu9250$ ./imucal -h
...
The default bus and sample rate are used.
        ...
        pi@raspberrypi:~/linux-mpu9250$ ./imucal -a
...
recording the min/max values.
        ...
        pi@raspberrypi:~/linux-mpu9250$ cat accelcal.txt 
        -16368
        16858
        -16722
        16644
...
Do the same thing for the magnetometers running <code>imucal</code> with the -m switch.
        ...
        pi@raspberrypi:~/linux-mpu9250$ ./imucal -m
...
will be written.
        ...
        pi@raspberrypi:~/linux-mpu9250$ cat magcal.txt 
        -179
        121
        -154
        199
...
<code>linux-mpu9250</code> code.

        pi@raspberrypi ~/linux-mpu9250 $ ./imu -h
...
up automatically.
        ...
        pi@raspberrypi ~/linux-mpu9250 $ ./imu
...
are available. See <code>mpu9250/mpu9250.c</code> for some examples.
